...
export default function LearnersLectureDetailPage() {
  ...
  if (!lectureDetail) {
    if (lectureFromEnrollments) {
      return (
        <div className="container mx-auto max-w-[1200px] space-y-6 px-8 py-8">
          <Title
            title={lectureFromEnrollments.title}
            description={`${displayLectureSubject} · ${displayInstructorName} 강사`}
          />
          ...
          <Card>
            <CardContent className="space-y-3 p-6">
              ...
              <div>
                <Button variant="outline" onClick={handleMoveList}>
                  목록으로 돌아가기
                </Button>
              </div>
            </CardContent>
        ...
        <CardContent className="p-8">
          <div className="flex items-center justify-between pb-16">
            ...
            {selectedExamIds.length > 0 && (
              <Button variant="outline" onClick={resetSelectedExamIds}>
                선택 초기화
              </Button>
            )}
          </div>
